...
def process_faces(frame, detections, executor_age_gender, resize_factor):
    global age_list, gender_list
    ...
    for detection in detections:
        ...
        input_tensors = ann.make_input_tensors([executor_age_gender.input_binding_info], [face_img])

        result = executor_age_gender.run(input_tensors)
        gender = gender_list[np.argmax(result[0][0])]
        age = age_list[np.argmax(result[1][0])]

        result_list.append([gender, age])

    return result_list

def draw_result(frame: np.ndarray, detections: list, resize_factor, face_data):
    """
    ...
    for i in range(len(detections)):
        ...
        color = (255, 0, 0) if face_data[i][0] == 'male' else (0, 0, 255)
        ...
        label = "Gender: {}, Age: {}".format(face_data[i][0], face_data[i][1])
...
def main(args):
    video, video_writer, frame_count = init_video_file_capture(args.video_file_path, 'age_gender_demo')
    frame_num = len(frame_count)

    executor_fd = ArmnnNetworkExecutor(args.first_model_file_path, args.preferred_backends)
    executor_age_gender = ArmnnNetworkExecutor(args.second_model_file_path, args.preferred_backends)    
    ...
    for _ in tqdm(frame_count, desc='Processing frames'):
        ...
        start_time = time.time() # measure only inference and intermediary processing times
        output_result = executor_fd.run(input_tensors)
        detections = process_output(output_result)
        face_data = process_faces(frame, detections, executor_age_gender, resize_factor)
        end_time = (time.time() - start_time)*1000

        draw_result(frame, detections, resize_factor, face_data)
